...
// 다른 class 에 extends 할 때의 모습
// class Admin extends User {
//   constructor(name: string, age: number, private permmission: string[]) {
//     super(name, age);
//   }
// }

// interface 의 쓰임
// type 명령어로 지정하는 것 외에 interface라는 명령어도 있다.
// interface CalculationContainer {
//   res: number;
//   print(): void;
// }

// type CalculatiionResults = CalculationContainer[];

// 이외에도 interface는 class에서 요긴하게 쓰인다
//  큰 프로젝트를 진행함에 있어서
// 어떤 함수나 클래스가 어떤 기능을 수행하는지 타입은 어떻게 정의됐는지를 일일이 찾는 것은 쉽지 않다.
// 허나 여기저기서 사용되는 기능 중 어떤 한 기능 가령 여기서는 Printable이 쓰였는지에 관심이 있다면
// 이런식으로 정의를 해두면 후에 이 클래스가 어떤 기능을 가지고 있는지 찾기 쉽고
// 해당 기능의 interface의 타임을  재정의하거나 하는 식으로 유지 보수가 쉬워진다.
// 그리고 클래스를 안전하고 보장되게 사용할 수 있다. ( interface로 지정하지 않고하면 에러가 날 우려가 있다.)
// **------------------------------------------------ //
interface Greetable {
  name: string;
}
...
function printResult<T>(result: T, printMode: OutputMode): void {
